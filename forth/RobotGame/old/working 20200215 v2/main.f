...
\ Other constants and variables
	...
	wordlist constant menu_char_wordlist	\ game word list stored in extra ROM
	wordlist constant menu_skills_wordlist	\ game word list stored in extra ROM
	wordlist constant menu_res_wordlist		\ game word list stored in extra ROM	
	variable rand_val
...
\ Tile data
	...
	\ GENERATED TILES (see asset extraction script for sizes below)
		...
		\ create tile_Menu_item_temp \ B2 allot
	...
	\ Menus
		DictCharMenu
			...
			: DrawMenuInventoryChar ( -- )
				...
				MENU_CHAR_BOX_Y_COUNT 0 do
					MENU_CHAR_BOX_X_COUNT 0 do
						MENU_CHAR_GRID_LEFT i 10 * + MENU_CHAR_GRID_TOP j 10 * +
						2dup Menu_item_slot DrawTile
						rot dup hero_inventory + c@ dup
						item_none <> if
							drop	\ item ID
							1+ -rot
			...
			: DrawCharMenu ( -- )
				...
				E const STAT_TEXT_DIST
				MENU_CHAR_HP_TOP		\ start Y for text
				HERO_STAT_COUNT 0 do
					[ hero_stat_texts 3 + ] literal i STAT_TEXT_DIST * +
					over STAT_TEXT_LENGTH swap MENU_CHAR_STAT_LEFT swap
					hero_stat_colors i + c@ COLOR_MENU_CHAR DrawText
			...
			: DrawMenuBorderChar ( -- )
				...
				s" [C]haracter" MENU_CHAR_LEFT MENU_TITLE_TOP COLOR_WHITE COLOR_MENU_CHAR DrawText
				s" S[k]ills" MENU_SKILL_LEFT MENU_TITLE_TOP COLOR_WHITE COLOR_MENU_SKILLS DrawText
				s" [R]esources" MENU_RES_LEFT MENU_TITLE_TOP COLOR_WHITE COLOR_MENU_RESOURCES DrawText
			...
			: KeyHandlerMenu ( -- )
				...
				output_mode c@ case
					OUTPUT_CHARACTER of
						...
						endcase
					endof
					OUTPUT_SKILLS of drop endof
					OUTPUT_RESOURCES of drop endof
				endcase
				;
