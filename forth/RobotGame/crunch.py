...
def assign(word,debug_file):
    ...
    if word.lower() in exception_words:
        ...
        enum_counter=0
        return ""
    elif word.lower()=="begin_var":
        var_counter=0
        var_number+=1
        return "create v"+str(var_number)
    elif word.lower()=="end_var":
        return str(hex(var_counter)[2:])+" allot"
    elif word.lower()=="lit":
        lit_mode=True
        return ""
        ...
        if lit_mode:
            lit_mode=False
            temp_ret={k.lower():v for (k,v) in const_words.items()}[word.lower()]
            if temp_ret[-10:]=="] literal ":
                temp_ret=temp_ret[2:-10]
            else:
                print("WARNING: lit mode set for non literal!",word.lower(),"=",temp_ret)
            return temp_ret
        else:
...
def main():
    ...
    for line in read_file.readlines():
        ...
        elif len(const_check)>=2 and const_check[1].lower()=="set_enum":
            enum_counter=int(const_check[0],16)
        elif len(const_check)>=2 and const_check[0].lower()=="enum":
            const_words[const_check[1]]=str(hex(enum_counter)[2:])
            enum_counter+=1
        elif len(const_check)>=2 and const_check[0].lower()=="var":
            const_words[const_check[1]]="[ v"+str(var_number)+" "+str(hex(var_counter)[2:])+" + ] literal "
            var_counter+=1
        else:
