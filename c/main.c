...
void ColorHero(void);
void ColorTile(int tile, int color_index);
void CopyTile(int dest, int src);
...
void DrawBorder(unsigned char x, unsigned char y, unsigned int width, unsigned char height, unsigned char color);
void DrawFrame(void);
void DrawItem(unsigned char tile, unsigned char x, unsigned char y);
void DrawItemStats(unsigned char item,unsigned char x,unsigned char y,unsigned char bg_color);
void DrawLegend(void);
void DrawMenu(void);
void DrawMenuBorder(void);
void DrawMenuInventory(void);
...
void DrawTextF(const unsigned char *msg, unsigned char uc1, unsigned char uc2, unsigned char x, unsigned char y, unsigned char fg_color, unsigned char bg_color);
void DrawTextNL(const unsigned char *msg, unsigned char x, unsigned char y, unsigned char height, unsigned char fg_color, unsigned char bg_color);
void DrawTile(int tile, int x, int y);
void DrawTile1bpp(unsigned char tile, unsigned char x, unsigned char y, unsigned char color1, unsigned char color2);
void DrawTileFull(int tile, int x, int y);
unsigned char DrawUC(unsigned char num, unsigned char x, unsigned char y, unsigned char fg_color, unsigned char bg_color);
void EnableBankeROM(void);
void EnableGfxRAM(void);
void main(void);
void PopBank3(void);
void PushBank3(void);
...
void UpdateTarget(void)
{
	...
	{
		if (hero_Y==FRAME_HEIGHT-1) return;
		ty++;
	}
	...
	{
		if (hero_Y==FRAME_WIDTH-1) return;
		tx++;
	}
...
void DrawSkills(void)
{
	...
	for (i=0;i<3;i++)
	{
		...
		for (j=0;j<5;j++)
		{
			...
			{
				DrawTile1bpp(index,j*MENU_SKILL_X_SPACING+MENU_SKILL_SKILL_LEFT,i*MENU_SKILL_Y_SPACING+MENU_SKILL_SKILL_TOP,COLOR_GRAY_1,COLOR_GRAY_2);
			}
