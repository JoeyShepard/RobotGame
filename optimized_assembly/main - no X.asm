...
;=====================
	ORG $200
	;Must come after include const.asm since relies on constants
	include globals.asm
...
;Graphics functions
;==================
	;Trad asm: 329k cycles vs 1.3m for MOVE in Forth
	;New asm: 328.9k, ie no real change
	FUNC clrscr
		ARGS
			BYTE color
		VARS
...
;==============
	...
	FUNC CopyTile
		...
		STA (dest_ptr),Y
		;STA row_count,X					;save row count
	...
	FUNC DrawTile
		ARGS
			BYTE tile, xpos, ypos
		VARS
			BYTE t_width, row_count, t_count, t_color
			WORD gfx_ptr, tile_ptr
			WORD addr_temp
		END
		...
		.dt.loop:
			...
			IF_0
				;DEC row_count,X				;decrease row_count
				DEC row_count
				BEQ .dt.return
			...
			ELSE_IF
				...
				ELSE_IF
					...
					.color.for:
						...
						DEY
					BPL .color.for
					;LDA t_count,X			;advance gfx_ptr by number of pixels drawn
	...
	FUNC RotateTile90
		ARGS
			BYTE dest, src
		VARS
			WORD gfx_ptr, dest_ptr
			BYTE t_width, row_count
			BYTE counter
			BYTE new_color,new_count
		END
		...
		.rt.loop_outer:
			;LDA t_width,X				;point to (0,width) pixel, ie bottom left pixel
			...
			.rt.loop_inner:
				...
				DEC counter
				BNE .rt.loop_inner
			;LDA new_count,X				;write pair followed by 0 for end of row
	...
	FUNC ColorTile
		...
		VARS
			WORD tile_ptr,color_table
			BYTE row_count,t_color,c_size,i
		END
...
;=================
	FUNC DrawRect	;Note width of 256 will come in as 256#256=0. still works with Y index
		...
		VARS
			WORD gfx_ptr
			BYTE x_end	;since can't do CPY addr,X
		END
		;LDA width,X
		LDA width
		BEQ .dr.done
	...
	FUNC DrawBar
		...
		VARS
			WORD bar_width, mult_buff, div_buff
		END
	...
	FUNC DrawLegend	
		VARS
			WORD gfx_ptr, map_ptr
			BYTE height, mX, mY, if_monster, if_crystal
		END
...
;==============
	...
	FUNC DrawSkills
		VARS
			...
			BYTE row, column, index, tleft, top, color, color2
		...
		.ds.rows:
			...
			.ds.columns:
				...
				IF_TRUE
					...
					;STA color2,X
					STA color2
					CALL DrawTile1bpp, index, tleft, top , color, color2
				ELSE_IF
					CALL DrawTile1bpp, index, tleft, top , #COLOR_GRAY_1, #COLOR_GRAY_2
				END_IF
				...
				IF_EQ
					...
					CMP row
					IF_EQ
						CALL DrawTile1bpp, #Menu_skill_selector, tleft, top, #COLOR_TRANSPARENT, #COLOR_GREEN
						...
						STA index_buff			;save index for skill description
						TAY
						LDA skill_titles,Y
						STA str_ptr
...
;	ENDM	
	...
	FUNC DrawResources
		VARS
			...
			BYTE tleft, top, color, color2, item
		...
		.dres.rows:
			...
			CALL DrawRect, tleft, #MENU_RES_BLOCK_TOP, #MENU_RES_BLOCK_WIDTH, #2+MENU_RES_BLOCK_HEIGHT*5, color
			...
			.dres.columns:
				...
				IF_LT
					...
					CALL DrawItem, tile_index, tleft, top
				...
				ELSE_IF
					...
					IF_LT
						...
						CALL DrawTile1bpp, item, tleft, top, color, color2
				...
				IF_EQ
					...
					IF_EQ
						...
						CALL DrawTile1bpp, #Menu_item_selector, tleft, top, #COLOR_TRANSPARENT, #COLOR_GREEN 
...
;==============
	FUNC InitHero
		...
		MOV.B #50,hero_Red
		MOV.B #50,hero_Blue
		MOV.B #50,hero_Yellow
		MOV.B #50,hero_Points
		MOV.B #false,hero_activity
...
;=============	
	FUNC main, begin
		...
		CALL ResetGame
		;halt
			;Traditional - 2,238,220
			;Optimized - 2,045,268
			;9.4%
		CALL DrawFrame
		;halt
			;Traditional - 2,781,907
			;Optimized - 2,558,651
			;9.19%
		CALL DrawLegend
		;halt
			;Traditional - 3,684,154
			;Optimized - 3,385,729
			;9.18%
